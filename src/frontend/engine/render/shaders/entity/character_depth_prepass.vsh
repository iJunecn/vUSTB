/**
 * @file character_depth_prepass.vsh
 * @brief 角色深度预通道顶点着色器
 * 输入：角色顶点、实例模型矩阵、动画参数、皮肤索引、CameraUniforms
 * 输出：alpha test 所需的 UV 与皮肤纹理层索引
 * 性能：仅计算位置与最小插值数据，为深度预写入服务
 */

#version 300 es

/* 精度约定：
 * highp 用于角色局部动画与视空间变换
 */
...
void main() {
    ...
    vec3 worldPosRel = worldPos.xyz - uViewPos.xyz; // 相机相对坐标，降低大坐标误差
